...
//for everything to run in order
db.serialize(() => {
  //creating tables if there arent any
...
//RSVP table creation if there arent any
...
app.get("/api/events", (req, res) => { 
  ...
  const params = [];
  let where = []; 
//catagory filter that shows when catagories are applied its added to the SQL query
  if (category) {
    where.push("category = ?");
    params.push(category);
  }
//date filter that shows when dates are applied user will only see events from those dates
  if (from) {
    where.push("date >= ?");
    params.push(from);
  }
...
//shows if filters exist on events this shows the event info, otherwise stays blank
...
//sql cmd to sort events by date, newest, or upcoming events in order
  db.all(sql, params, (err, rows) => {
    if (err) {
      ...
      res.json(rows); //filtered/approved events list
    }
  });
});
...
app.put("/api/events/:id", (req, res) => {
  ...
  db.run(
    ...
    function (err) {
      if (err) {
        console.error(err);
        res.status(500).json({ error: "Failed to update event" }); //failed to update event 
      } else {
...
//admin can delete events if something inappropriate/wrong is submitted
app.delete("/api/events/:id", (req, res) => {
  db.run("DELETE FROM events WHERE id=?", [req.params.id], function (err) {
    if (err) {
      console.error(err);
      res.status(500).json({ error: "Failed to delete event" }); //failed to delete event
    } else {
...
app.post("/api/events/:id/approve", (req, res) => {
  db.run("UPDATE events SET status='approved' WHERE id=?", [req.params.id], function (err) {
    ...
    res.json({ success: true }); //shows that the event is approved and shows for all roles
  });
});
...
app.post("/api/events/:id/reject", (req, res) => {
  db.run("UPDATE events SET status='rejected' WHERE id=?", [req.params.id], function (err) {
    ...
    res.json({ success: true }); //shows that the event is rejected does not show/is deleted
  });
});
...
app.post("/api/events/:id/rsvp", (req, res) => {
  ...
  db.run(
    ...
    function (err) {
      if (err) {
        console.error(err);
        res.status(500).json({ error: "Failed to RSVP" }); //failed to RSVP
      } else {
...
app.post("/api/events/:id/cancel-rsvp", (req, res) => {
  ...
  db.run(
    ...
    function (err) {
      if (err) {
        console.error(err);
        res.status(500).json({ error: "Failed to cancel RSVP" }); //failed to cancel RSVP
      } else {
